...
. *Parse YAML* -- Decode raw YAML into intermediate representation
. *Normalize* -- Canonicalize provider names, resolve aliases
. *Resolve References* -- Expand `${ref:path}`, `${env:VAR}`, `${file:path}` with dependency graph and cycle detection
. *Apply Defaults* -- Hydrate empty fields from provider-region defaults registry
. *Validate* -- Schema + business rules + provider + deployment + services checks
...
. Cluster config file
. CLI settings file (`~/.config/opencenter/config.yaml`)
. Built-in defaults
...
. *Schema Validation:* JSON schema compliance (structure, types, formats)
. *Business Rules:* Cross-field dependencies (e.g., VRRP IP required when Octavia disabled)
. *Provider Validation:* Provider-specific constraints (image IDs, flavors, networks)
. *Connectivity Validation:* API reachability and credential verification (optional)
...
. *`internal/sops/`* -- Low-level SOPS/Age encryption operations (encrypt/decrypt files, key generation, OS keyring integration)
. *`internal/secrets/`* -- High-level multi-cluster secrets management (sync, drift detection, rotation, revocation, registry, Git hooks)
...
. *`App` struct* (preferred) -- Explicit constructor chaining with typed fields. Built via `di.NewApp(baseDir)`.
. *`DIContainer`* (legacy) -- Reflection-based resolution matching constructor parameter types to registered return types.
...
For a complete architectural map of all packages, see xref:index.adoc[Codemaps Index].
...
| CLI
| `cmd/` (Cobra commands), `internal/ui` (prompts), `internal/plugins` (external plugins)

| Domain
| `internal/cluster` (lifecycle), `internal/secrets` (secrets mgmt), `internal/operations` (drift, backup)

| Config
| `internal/config` (types, loader, builder, v2), `internal/config/services` (service registry)

| GitOps
| `internal/gitops` (pipeline, templates, rendering), `internal/template` (engine)

| Infra
| `internal/cloud` (providers), `internal/provision` (templates), `internal/tofu` (OpenTofu), `internal/ansible` (Kubespray)

| Security
| `internal/security` (audit, masking, sanitization), `internal/sops` (encryption)

| Foundation
| `internal/di` (DI container), `internal/core` (paths, validation), `internal/util` (shared), `internal/resilience` (locks, retry)
|===
...
. Create provider adapter in `internal/cloud/<provider>/`
. Implement provisioning logic in `internal/provision/<provider>/`
. Add provider-specific validation
. Update schema with provider configuration
...
. Create service configuration in `internal/config/services/<service>.go`
. Add service to defaults in `internal/config/defaults.go`
. Create service manifests in openCenter-gitops-base
...
. Implement validator interface in `internal/core/validation/validators/`
. Register validator with validation engine
...
. Create executable named `opencenter-<plugin>`
. Place in PATH
. CLI discovers and loads automatically
...
* xref:concepts/gitops-workflow.adoc[GitOps Workflow] - Repository structure and reconciliation
* xref:concepts/security-model.adoc[Security Model] - Security architecture and controls
* xref:concepts/configuration-lifecycle.adoc[Configuration Lifecycle] - Configuration management
* xref:concepts/provider-comparison.adoc[Provider Comparison] - Choosing infrastructure providers
...
For detailed code-level architecture maps, see xref:index.adoc[Codemaps].
