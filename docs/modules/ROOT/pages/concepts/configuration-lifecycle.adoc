...
. *Single Source of Truth:* One YAML file defines entire cluster
. *Version Controlled:* Configuration stored in Git
. *Declarative:* Describe desired state, not steps
. *Validated:* Multi-layered validation before deployment
...
. *Direct Editing:* Edit configuration file with text editor
. *Interactive Mode:* `opencenter cluster edit my-cluster`
. *CLI Flags:* `opencenter cluster set my-cluster cluster.worker_count=5`
...
. *Command-line flags:* `opencenter cluster set my-cluster cluster.worker_count=5`
. *Configuration file:* `~/.config/opencenter/clusters/my-org/.my-cluster-config.yaml`
. *CLI defaults:* `~/.config/opencenter/config.yaml`
. *Built-in defaults:* `internal/config/defaults.go`
...
. *Infrastructure Drift:* VMs deleted, networks changed, storage modified
. *Configuration Drift:* Manual kubectl changes, direct API modifications
. *Service Drift:* Service versions changed, settings modified
...
* xref:concepts/architecture.adoc[Architecture] - System design and components
* xref:concepts/gitops-workflow.adoc[GitOps Workflow] - Repository structure and reconciliation
* xref:operations/validate-configuration.adoc[Validate Configuration] - Validation procedures
* xref:operations/troubleshoot-deployment.adoc[Troubleshoot Deployment] - Fix deployment issues
* xref:reference/configuration-schema.adoc[Configuration Schema] - Complete field reference
