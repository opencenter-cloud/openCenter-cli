...
For that separate mechanism, see xref:concepts/plugin-external-cli.adoc[Plugin External CLI].
...
. CLI wiring for `cluster service enable`, `cluster service options`, and service-specific secrets.
...
. Set `Enabled = true`.
. Apply `--param` values by matching CLI keys to JSON tags on struct fields.
. Apply `--secret` values by routing into the service's secret struct.
. Run service-specific checks.
. Save the updated cluster config.
. Optionally render just that service with `--render`.
...
. Render the cluster application overlays from `templates/cluster-apps-base`.
...
* xref:concepts/services-templates.adoc[Service Templates]
