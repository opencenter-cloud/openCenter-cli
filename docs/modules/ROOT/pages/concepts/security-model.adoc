...
. *disallow-host-namespaces:* Block host PID/IPC/network
. *disallow-host-path:* Block host path volumes
. *require-run-as-nonroot:* Require non-root user
. *restrict-seccomp:* Require seccomp profile
. *restrict-volume-types:* Limit volume types
. *disallow-capabilities:* Drop dangerous capabilities
. *require-ro-rootfs:* Require read-only root filesystem
. *disallow-host-ports:* Block host port binding
. *require-probes:* Require liveness/readiness probes
. *restrict-image-registries:* Limit allowed registries
. *require-labels:* Require standard labels
. *disallow-latest-tag:* Block :latest image tag
. *require-requests-limits:* Require resource limits
. *disallow-default-namespace:* Block default namespace
. *require-network-policy:* Require NetworkPolicy
. *disallow-deprecated-apis:* Block deprecated APIs
...
* xref:operations/manage-secrets.adoc[Manage Secrets] - SOPS and secrets management
* xref:operations/customize-services.adoc[Customize Services] - Security service configuration
