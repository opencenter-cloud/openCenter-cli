...
* Development environment set up (see xref:contributing/development-setup.adoc[Development Setup])
...
. *Configuration defaults* - Default values for provider-specific settings
. *Validation logic* - Provider-specific validation rules
. *Preflight checks* - Connectivity and credential validation
. *Templates* - Infrastructure-as-code templates (Terraform/Pulumi)
. *Documentation* - Provider-specific guides
