...
| Helm Controller
| `kubectl get deploy helm-controller -n flux-system`
| `READY 1/1`
...
| Kustomization
| `flux get kustomization envoy-gateway-api-base`
| `READY=True`
...
| Kustomization
| `flux get kustomization metallb-base`
| `READY=True`
...
| Kustomization
| `flux get kustomization gateway`
| `READY=True`
...
| Address
| `kubectl get gateway -n gateway-system -o jsonpath='{.items[0].status.addresses}'`
| Has IP
...
| gateway
| cert-manager-base, envoy-gateway-api-base
| gateway-system
...
| sources, observability-namespace
| observability
