...
| no
| no
| `true` on `opencenter.services.harbor.enabled`
...
| no
| no
| `true` on `opencenter.services.etcd-backup.enabled`
...
| yes
| no
| `true` on `opencenter.services.sealed-secrets.enabled`
...
| yes
| yes
| `true` on `opencenter.services.kyverno.enabled`
...
| yes
| yes
| `true` on `opencenter.services.longhorn.enabled`
|===
...
| present in all clusters, aliased to cluster-specific names in fixture
| template renders cluster-specific content; no condition needed

| `opencenter-aws-credentials-secret.yaml`
| present only when AWS credentials are configured
| `exists` on `secrets.global.aws.application.access_key`

| `rackspace-selfsigned-ca.yaml`
| present in k8s-prod, k8s-uat
| `true` on a cert-manager config field
|===
...
| `20-keycloak/patch-subscription.yaml`
| present in clusters using OLM-managed keycloak
| `true` on `opencenter.services.keycloak.pin_operator_version` or similar

| `20-keycloak/rbac-manager-users.yaml`
| absent from k8s-dr fixture
| fixture drift (ignored in canonicalization)
|===
...
| present in all clusters, aliased to `alertmanager-security-policy.yaml` in some
| template renders cluster-specific content; alias in canonicalization
|===
...
| `rbac.yaml`, `rbac2.yaml`
| present in some clusters
| fixture drift (ignored in k8s-dr canonicalization)
|===
...
| present in clusters with delete reclaim policy
| `equals` on a vsphere-csi config field

| `storageclass-retain.yaml`
| present in clusters with retain reclaim policy
| `equals` on a vsphere-csi config field
|===
...
| `true` on `opencenter.gitops.overlay_units.customer_managed.enabled`

| Secret manifest in sources
| present only in k8s-uat
| `true` on `opencenter.gitops.overlay_units.customer_managed.emit_secret`
|===
...
| file presence
| present in k8s-dev, k8s-dr; absent from k8s-prod, k8s-qa, k8s-uat
| `true` on `opencenter.gitops.overlay_units.sops.enabled`
|===
