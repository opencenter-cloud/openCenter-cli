...
. *_Mise installed_* - Tool version manager and task runner
+
[,bash]
----
# macOS
...
. *_Git installed_* - Version control
+
[,bash]
----
...
. *_Infrastructure access_* - One of:
...
See xref:reference/platform-services.adoc[Platform Services Reference] for complete list.
...
. *_Provisions infrastructure_* - Creates VMs, networks, load balancers (OpenStack/AWS)
. *_Deploys Kubernetes_* - Runs Kubespray Ansible playbooks
. *_Bootstraps FluxCD_* - Installs GitOps controller
. *_Deploys services_* - FluxCD reconciles platform services from gitops-base
...
* *_xref:operations/add-worker-pools.adoc[Add Worker Pools]_* - Scale your cluster
* *_xref:operations/manage-secrets.adoc[Manage Secrets]_* - Rotate encryption keys
* *_xref:operations/customize-services.adoc[Customize Services]_* - Configure platform services
* *_xref:index.adoc[Deploy Applications]_* - Add your workloads
...
See link:../how-to/troubleshoot-deployment.md#validation-errors[Troubleshooting Guide] for details.
...
See xref:operations/troubleshoot-deployment.adoc[Troubleshooting Guide] for complete solutions.
