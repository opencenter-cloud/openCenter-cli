...
. Open vSphere Client
. Right-click datacenter → New Virtual Machine
. Create 6 VMs with specifications above
. Install Ubuntu 24.04 on each VM
...
. VMware constraints (VM inventory, vSphere credentials)
. SSH connectivity (can reach all VMs)
...
* xref:operations/customize-services.adoc[Customize Services] - Configure platform services
* xref:operations/manage-secrets.adoc[Manage Secrets] - Encrypt and rotate secrets

*Cluster Management:*

* xref:operations/add-worker-pools.adoc[Add Worker Pools] - Scale cluster capacity
* xref:operations/backup-and-restore.adoc[Backup and Restore] - Configure disaster recovery
* xref:operations/upgrade-kubernetes.adoc[Upgrade Kubernetes] - Upgrade cluster version
...
* xref:concepts/gitops-workflow.adoc[GitOps Workflow] - How GitOps works
* xref:concepts/provider-comparison.adoc[Provider Comparison] - Compare providers
