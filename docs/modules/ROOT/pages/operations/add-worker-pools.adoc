...
. *Use descriptive pool names:* `highmem-pool`, `gpu-pool`, not `pool-1`, `pool-2`
...
* xref:operations/configure-networking.adoc[Configure Networking] - Network configuration for worker pools
* xref:operations/upgrade-kubernetes.adoc[Upgrade Kubernetes] - Upgrade nodes in worker pools
* xref:operations/troubleshoot-deployment.adoc[Troubleshoot Deployment] - Debug node issues
* xref:reference/configuration-schema.adoc[Configuration Schema] - Complete configuration reference
