...
* xref:operations/upgrade-kubernetes.adoc[Upgrade Kubernetes] - Backup before upgrades
* xref:operations/migrate-clusters.adoc[Migrate Clusters] - Use backups for migration
* xref:operations/troubleshoot-deployment.adoc[Troubleshoot Deployment] - Restore from backup
* xref:reference/platform-services.adoc[Platform Services] - etcd-backup and Velero configuration
