...
For background on how the plugin mechanism works, see xref:concepts/plugin-external-cli.adoc[Plugin External CLI].
...
. `OPENCENTER_PLUGINS_DIR` environment variable
. `~/.config/opencenter/plugins/` (default config-based path)
...
| `verified`
| Checksum matches `checksums.txt` entry
| Runs normally

| `unverified`
| No entry in `checksums.txt`
| Runs with a warning on stderr

| `checksum-mismatch`
| Entry exists but hash differs
| Execution blocked
...
* xref:concepts/plugin-external-cli.adoc[Plugin External CLI] -- how the discovery and execution model works
* xref:concepts/plugin-internal-services.adoc[Plugin Internal Services] -- for adding platform services (cert-manager, Loki, etc.)
* xref:reference/file-locations.adoc[File Locations] -- plugins directory and checksums.txt location
* xref:reference/cli-commands.adoc[CLI Commands] -- `opencenter plugin list` reference
