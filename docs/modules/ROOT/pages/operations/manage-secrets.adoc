...
* xref:operations/customize-services.adoc[Customize Services] - Configure platform services with encrypted secrets
* xref:operations/backup-and-restore.adoc[Backup and Restore] - Include encryption keys in backups
* xref:operations/troubleshoot-deployment.adoc[Troubleshoot Deployment] - Fix SOPS-related issues
