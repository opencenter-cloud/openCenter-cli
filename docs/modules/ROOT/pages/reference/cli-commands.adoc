...
| `--output`
| Select output format: `text`, `json`, `yaml`
...
| `opencenter cluster`
| Cluster lifecycle, validation, rendering, drift, services, backup, and import management

| `opencenter secrets`
| Secret encryption, sync, validation, and key operations

| `opencenter settings`
| CLI settings, defaults, and local IDE configuration
...
| Session-scoped shell integration for active-cluster context

| `opencenter <external-plugin>`
| Dynamically discovered plugin entrypoints such as `rmpk`
|===
