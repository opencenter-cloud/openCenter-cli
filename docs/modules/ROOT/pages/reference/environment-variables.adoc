...
. Command-line flags (the set override mechanism, `--log-level`, etc.)
. Environment variables
. Cluster config file (`.<cluster>-config.yaml`)
. CLI config file (`~/.config/opencenter/config.yaml`)
. Built-in defaults

For the full breakdown including directory resolution and provider credentials, see xref:reference/configuration-precedence.adoc[Configuration Precedence].
...
. *Use configuration files for persistent settings:* Environment variables for temporary overrides only
. *Never commit credentials:* Use SOPS encryption or secret management
. *Use CI/CD secrets:* For credentials in pipelines
. *Document required variables:* In README or CI/CD configuration
. *Use descriptive names:* `OPENCENTER_*` prefix for clarity
. *Validate before use:* Check environment variables are set correctly
. *Unset after use:* Clean up temporary variables
...
* xref:reference/cli-commands.adoc[CLI Commands] - Command-line flags
* xref:operations/integrate-ci-cd.adoc[Integrate CI/CD] - CI/CD integration
