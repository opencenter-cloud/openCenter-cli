...
*Purpose:* 32-byte HMAC-SHA256 key used to sign audit log entries for tamper detection. Generated automatically on first audit write. See xref:reference/audit-key.adoc[Audit Signing Key] for details.
...
*See also:* xref:operations/create-install-cli-plugin.adoc[Create and Install a CLI Plugin]
...
| `${OPENCENTER_CONFIG_DIR:-~/.config/opencenter}/clusters`

| Plugins directory
| `OPENCENTER_PLUGINS_DIR`
| `${OPENCENTER_CONFIG_DIR:-~/.config/opencenter}/plugins`
...
| Runtime logs
| `0600`
| May contain command output and error details

| Runtime state
| `0600`
| Resume checkpoints may contain sensitive context
...
. *Cluster configurations:* `~/.config/opencenter/clusters/`
. *SOPS Age keys:* `~/.config/opencenter/clusters/*/secrets/age/`
. *SSH keys:* `~/.config/opencenter/clusters/*/secrets/ssh/`
...
* xref:reference/configuration-schema.adoc[Configuration Schema] - Configuration file structure
* xref:reference/cli-commands.adoc[CLI Commands] - Command reference
* xref:reference/environment-variables.adoc[Environment Variables] - Environment variable reference
* xref:operations/manage-secrets.adoc[Manage Secrets] - Secrets management
