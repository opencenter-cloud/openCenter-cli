...
| Kubespray, Kamaji
| Detect only
| Canonical name is `vmware`; `vsphere` is an alias
...
| Kind bootstrap flow
| Not applicable
| Use for development and CI only
...
| Kubespray
| Not applicable
| Manual provisioning and host lifecycle

| AWS
| Non-GA infrastructure provider
| Not supported for GA cluster provisioning
| N/A
| Removed from drift registry
| AWS service integrations remain supported where used by platform services
|===
