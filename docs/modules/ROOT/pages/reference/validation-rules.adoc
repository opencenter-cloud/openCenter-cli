...
. *Schema Validation:* Structure, types, formats (JSON schema)
. *Business Rules:* Cross-field dependencies, logical consistency
. *Provider Validation:* Provider-specific constraints
. *Connectivity Validation:* API reachability (optional)
...
| `opencenter.cluster.master_count`
| integer
| 3

| `opencenter.cluster.worker_count`
| integer
| 2
...
| `opencenter.services.cert-manager.enabled`
| boolean
| true
...
| `opencenter.infrastructure.openstack.auth_url`
| URL
| "https://..."

| `opencenter.cluster.networking.pod_subnet`
| CIDR
| "10.42.0.0/16"

| `opencenter.cluster.networking.service_subnet`
| CIDR
| "10.43.0.0/16"

| `opencenter.services.keycloak.hostname`
| hostname
| "auth.example.com"
|===
...
| `opencenter.infrastructure.provider`
| "openstack", "vmware", "kind", "aws", "baremetal"

| `opencenter.cluster.networking.cni_plugin`
| "calico", "cilium", "kube-ovn"

| `opencenter.meta.environment`
...
* xref:operations/troubleshoot-deployment.adoc[Troubleshoot Deployment] - Fix validation errors
